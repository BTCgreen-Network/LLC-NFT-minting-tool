...
from littlelambocoin.wallet.did_wallet.did_wallet_puzzles import LAUNCHER_PUZZLE_HASH
from littlelambocoin.wallet.trading.offer import Offer
...
class Minter:
    ...
    async def get_wallet_ids(
        ...
        llc_wallets = await self.wallet_client.get_wallets(
            wallet_type=WalletType.STANDARD_WALLET
        )
        self.llc_wallet_id = llc_wallets[0]["id"]

    async def get_funding_coin(self, amount: int) -> Coin:
        coins = await self.wallet_client.select_coins(amount=amount, wallet_id=self.llc_wallet_id)  # type: ignore
        if len(coins) > 1:
            raise ValueError(
    ...
    async def create_spend_bundles(
        ...
        for i in range(0, mint_total, chunk):
            resp = await self.wallet_client.nft_mint_bulk(
                ...
                llc_coins=[next_coin.to_json_dict()],
                llc_change_target=next_coin.to_json_dict()["puzzle_hash"],
    ...
    async def submit_spend_bundles(
        ...
        for i, sb in enumerate(spend_bundles):
            llc_coin_to_spend = [coin for coin in sb.removals() if coin.amount > 1][0]
            coin_record = await self.node_client.get_coin_record_by_name(
                llc_coin_to_spend.name()
            )
        ...
        if total_fee_to_pay > 0:
            fee_coins = await self.wallet_client.select_coins(  # type: ignore
                ...
                wallet_id=self.llc_wallet_id,
                excluded_coins=[llc_coin_to_spend],
            )
        ...
        for i, sb in enumerate(spend_bundles[starting_spend_index:]):
            ...
            if create_sell_offer:
                ...
                for launcher_id in launcher_ids:
                    offer_dict = {
                        launcher_id: -1,
                        self.llc_wallet_id: int(create_sell_offer),
                    }
                    try:
