...
from littlelambocoinnft import __version__
from littlelambocoinnft.util.mint import Minter

if os.environ.get("TESTING_CIC_CLI", "FALSE") == "TRUE":
    from tests.cli_clients import get_node_and_wallet_clients
else:
    from littlelambocoinnft.util.clients import get_node_and_wallet_clients
...
@click.group(
    help="\n  NFT minting for Littlelambocoin Blockchain \n",
...
@click.option(
    ...
    required=False,
    default="",
    help="A standard LLC address where royalties will be sent",
)
@click.option(
    "-r",
    "--royalty-percentage",
