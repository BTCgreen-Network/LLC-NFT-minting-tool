...
class WalletClientMock:
    ...
    async def _assign_did(self):
        ...
        self.did_id = encode_puzzle_hash(self.did_coin.name(), "did:littlelambocoin:")

    async def get_nft_wallet_did(self, wallet_id):
        did_addr = encode_puzzle_hash(self.did_coin.puzzle_hash, "did:littlelambocoin:")
    ...
    async def get_next_address(self, wallet_id: int, new_address: bool) -> str:
        return encode_puzzle_hash(bytes32(token_bytes(32)), "llc")
    ...
    async def nft_mint_bulk(
        ...
        llc_coins: Optional[List[Dict]] = None,
        llc_change_target: Optional[str] = None,
        ...
        spend_bundles = []
        # construct a spendbundle using llc coin and did coin, spending both back to themselves
        assert isinstance(llc_coins, List)
        llc_coin = llc_coins[0]
        llc_conds = [[51, bytes32.from_hexstr(llc_coin["puzzle_hash"]), int(llc_coin["amount"])]]  # type: ignore
        llc_spend = CoinSpend(Coin.from_json_dict(llc_coin), ACS, Program.to(llc_conds))
        if mint_from_did:
            ...
            spend_bundles.append(SpendBundle([llc_spend, did_spend], G2Element()))
        else:
            spend_bundles.append(SpendBundle([llc_spend], G2Element()))
        return {
            "success": True,
