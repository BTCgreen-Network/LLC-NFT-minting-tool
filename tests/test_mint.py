...
from littlelambocoinnft.cmds.cli import cli
...
def create_metadata(filename: str, mint_total: int, has_targets: bool) -> str:
    ...
    for i in range(mint_total):
        ...
        if has_targets:
            sample.append(encode_puzzle_hash(bytes32(token_bytes(32)), "llc"))
    ...
    with runner.isolated_filesystem():
        ...
        sb_result: Result = runner.invoke(
            ...
            [
                ...
                True,
                "--royalty-address",
                encode_puzzle_hash(bytes32(token_bytes(32)), "llc"),
                "--royalty-percentage",
                300,
                "--has-targets",
                has_targets,
...
@pytest.mark.parametrize("has_targets", [True, False])
def test_mint_from_llc(has_targets):
    mint_total = 10
    chunk_size = 5
