...
async def create_nft_sample(has_targets: bool) -> List[Any]:
    ...
    if has_targets:
        sample.append(encode_puzzle_hash(bytes32(token_bytes(32)), "tllc"))
    return sample


async def create_target_sample() -> List[Any]:
    return [encode_puzzle_hash(bytes32(token_bytes(32)), "tllc")]
...
async def main(count: int, has_targets: bool) -> None:
    ...
    royalty_address = encode_puzzle_hash(bytes32(token_bytes(32)), "tllc")
